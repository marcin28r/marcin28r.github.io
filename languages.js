const language_content = {
    "en": {
      ...
      "rrule-1": "move n - 1 disks from the starting pole to the auxiliary stack",
      "rrule-2": "move disk n from the starting pole to the target stack",
      "rrule-3": "move the disks from the auxiliary pole to the target stack, covering the n'th disk",
      ...
      "appl-2": "displaying discs - one of three options for displaying discs (wooden, colorful, patterned) can improve their visibility or simply make stacking more enjoyable",
      "appl-3": "single hint - the option after pressing the \"hint\" button suggests the next move, which is the most optimal in the current situation, firstly highlights in blue the stick from which the disc should be taken, and then the one on which we should put it back",
      "appl-4": "indicating posible moves - an option that highlights the stacks on which we can put the currently picked up disc in green, the indication is active passively, this option may be especially helpful for people who are just getting used to the rules",
      "appl-5": "automatic solving - the option provides the \"auto\" button, after activation of which the application will try to solve the puzzle itself, it should be noted that each of its moves is the most optimal in a given situation, when the option is active the button lights up green and turning it off can be initiated by clicking the button again, the application will complete the current move and stop",
      "appl-6": "map of moves - provides a panel in which all possible arrangements of the current puzzle are arranged, there is also a \"map\" button which increases the map display area",
      "appl-7": "2 players - option available and suggested only on wide screens, allows you to display two similar puzzles side by side, please note that any of the return options ends both of them",
    ...
    "pl": {
      ...
      "app-2": "Cały proces przekładania polega na klikaniu w trzy panele ze stosami. Kiedy klikniemy na konkretny stos, krążek z jego szczytu zostanie zawieszony nad nim. Kiedy natomiast jeden krążek jest już w górze, a my klikniemy na kolejny stos, krążek zostanie na nim umieszczony. Oczywiście istotne aby przełożenie uwzględniało zasady. W przeciwnym wypadku, niedozwolony ruch zostanie zasygnalizowany. Stos, na którym aktualne chcieliśmy położyć za duży krążek, podświetli się kolorem czerwonym. Zawsze widoczny jest również panel który wyświetla informacje o danej parti. Z lewej widnieją numery, które odzwierciedlają ilość ruchów wykonanych w stosunku do tych optymalnych. Po prawej stronie możemy śledzić upływający czas. Kiedy układanka zostanie ułożona panel informacyjny podświetli się na zielono, a zaraz po ty pokaże się panel z podsumowaniem. Z penelu możemy wybrać jedną z trzech opcji. Pierwsza \"Jeszcze raz\" pozwoli układać łamigłówke w tej samej konfiguracji. Druga przeniesie nas do konfiguratora, a ostatnia do strony początkowej.",
      "apph3": "Konfigurator pozwala na dostosowanie:",
      "appl-1": "ilości krążków - należy pamiętać, że im więcej krążków tym większa liczba ruchów potrzebna do ułożenia łamigłówki, przez co poziom trudności jest zwiększony",
      "appl-2": "wyświatlania krążków - jedna z trzech opcji wyświetlania krążków (drewniane, kolorowe, we wzory) może poprawić ich widoczność lub po prostu uprzyjemnić układanie",
      "appl-3": "pojedyncza podpowiedź - opcja po wciśnięciu przycisku \"wskazówka\" sugeruje kolejne przełożenie, które w aktualnej sytuacji jest tym najbardziej optymalnym, kolejno podświetla w kolorze niebieskim palk, z którego należy pobrać krążek, a następnie ten na który powinniśmy go odłożyć",
      "appl-4":  "wskazywanie poprawnego odkładania - opcja która podświetla stosy na które możemy odłożyć aktualnie podniesiony krążek kolorem zielonym, wskazywanie jest aktywne pasywnie, opcja ta może być szczególnie pomocna dla osób, które  dopiero oswajają się z zasadami",
      "appl-5": "automatyczne układanie - opcja udostępnia przycisk \"auto\", po którego aktywacji aplikacja będzie sama dążyć do ułożenia łamigłówki, należy zaznaczyć, że każdy z jej ruchów jest tym najbardziej optymalnym w danej sytuacji, kiedy opcja jest aktywna przycisk świeci się na zielono, a wyłączenie jej można zainicjować klikając na przycisk ponownie, aplikacja dokończy aktualne przełożenie i zakończy działanie",
      "appl-6": "mapa ruchów - udostępnia panel w którym rozmieszczone są wszystkie możliwe ułożenia aktualnej łamigłówki, dostępny jest również przycisk \"mapa\", który zwiększa obszar wyświetlaina mapy",
      "appl-7": "2 graczy - opcja dostępna i sugerowana jedynie na szerokich ekranach, pozwala na wyświetlenie dwóch podobnych łamigłówwek jedna obok drugiej, należy zwrócić uwagę, że którakolwiek z opcji powrotu kończy działanie obu z nich",
